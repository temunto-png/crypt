"""Live trading エンジンの状態永続化レイヤー。

SQLite の live_state テーブルに PortfolioState と実行状態をまとめて保存する。
...
@dataclass
class LiveState:
    """SQLite から読み書きされる live trading の完全な状態。"""
...
class LiveStateStore:
    """live_state テーブルへの CRUD を担う I/O レイヤー。
    ...
    def load(self) -> LiveState | None:
        ...
        with self._storage._connect() as conn:
            row = conn.execute(
                "SELECT * FROM live_state WHERE id = ?", (_STATE_ID,)
            ).fetchone()
        if row is None:
            return None
        d = dict(row)
    ...
    # ------------------------------------------------------------------
    # 変換ヘルパー（PaperStateStore と同一インターフェース）
    # ------------------------------------------------------------------

    @staticmethod
    def to_portfolio_state(state: LiveState) -> PortfolioState:
        """LiveState → PortfolioState に変換する。"""
    ...
    @staticmethod
    def extract_pending_signal(state: LiveState) -> Signal | None:
        """LiveState から pending Signal を復元する。"""
    ...
    @staticmethod
    def extract_recent_returns(state: LiveState) -> list[float]:
        """LiveState から recent_trade_returns を復元する。"""
        try:
    ...
    @staticmethod
    def extract_position_entry_time(state: LiveState) -> datetime | None:
        """LiveState から position_entry_time を復元する。"""
